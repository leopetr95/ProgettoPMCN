...
struct node  {//struttura di un nodo della lista dinamica ordianta
    ...
    char task_type;//tipo di job:1 o 2
    double time_arrive;//istante di arrivo del job
    double time_lost_in_cloudlet;
...
struct node* get_new_node(double instant_service,char task_type,double time_arrive,double time_lost_in_cloudlet);
...
void insert_ordered(double instant_service,char task_type,double time_arrive,double time_lost_in_cloudlet, struct node** head, struct node** tail);
...
int delete_head(struct node** head,double *time_arrive,double *time_lost_in_cloudlet);
