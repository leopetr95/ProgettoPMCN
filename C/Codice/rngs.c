 ...
 *  e if x > 0 then x is the state (unless too large)
 *    if x < 0 then the state is obtained from the system clock
 *    if x = 0 then the state is to be supplied interactively
 * ---------------------------------------------------------------
 */
...
{
    if(x==NULL){
        handle_error_with_exit("error in GetSeed");
    }
    *x = seed[stream];
}
...
{
    if(index<0){
        handle_error_with_exit("error in SelectStream\n");
    }
    stream = ((unsigned int) index) % STREAMS;
    if ((initialized == 0) && (stream != 0))   /* protect against        */
        PlantSeeds(SEED_DEFAULT);                     /* un-initialized streams */
}
